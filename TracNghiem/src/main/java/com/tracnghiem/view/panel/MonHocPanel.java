...
public class MonHocPanel extends javax.swing.JPanel {
    ...
    private void loadDataSubjectTable() {
        ...
        for (TopicDTO rootTopic : rootTopics) {
            ...
            rootRow[4] = rootTopic.getTpStatus() == 1 ? "Hoạt động" : "Không hoạt động"; // "Trạng thái"
            ...
            for (TopicDTO level1Topic : level1Topics) {
                ...
                level1Row[4] = level1Topic.getTpStatus() == 1 ? "Hoạt động" : "Không hoạt động"; // "Trạng thái"
                ...
                for (TopicDTO level2Topic : level2Topics) {
                    ...
                    level2Row[4] = level2Topic.getTpStatus() == 1 ? "Hoạt động" : "Không hoạt động"; // "Trạng thái"
    ...
    private void jButton6ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton6ActionPerformed
        ...
        JDialog dialog = mainView.showCustomDialog(null, panel, "Thêm môn học");
...
//                        model.setValueAt(i + 1, i, 0);
    ...
    private TopicDTO getTopic(int parentId) {
        ArrayList<TopicDTO> topics = tpBUS.getAll();
        List<TopicDTO> children = new ArrayList<>();

        // Tìm tất cả topic con của parentId
        for (TopicDTO topic : topics) {
            if (topic.getTpParent() == parentId) {
                children.add(topic);
            }
        ...
        if (children.isEmpty()) {
            TopicDTO topic = tpBUS.findOne(parentId);
            return topic;
        }

        // Tìm topic có ID nhỏ nhất trong danh sách con
        TopicDTO smallestChild = Collections.min(children, Comparator.comparingInt(TopicDTO::getTpID));

        // Đệ quy kiểm tra xem nó có con hay không
        return getTopic(smallestChild.getTpID());
    }
    ...
    private void jButton7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton7ActionPerformed
        ...
        int selectedTopicId = (int) table_monhoc.getValueAt(selectedRow, 0);
        
        
        addSubject panel = new addSubject(getTopic(selectedTopicId), true);

        // Gọi showCustomDialog() và lấy JDialog
        JDialog dialog = mainView.showCustomDialog(null, panel, "Sửa môn học");
